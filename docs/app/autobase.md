This module sets up **Autobase** for syncing ToDos.

Create a new file named `autobase-setup.js` in the project directory
```bash
touch src/autobase-setup.js
```

Select and copy the full content below and paste it inside `autobase-setup.js` file in your Pear app!
...
#### Exports:
- `setupAutobase(existingKey)` – Initializes Autobase.
- `appendToAutobase(operation)` – Appends new ToDo operations.
- `addWriter()` – Grants write access to peers.
- `apply()` – Reducer to handle ToDo operations or writer access.

#### Internals:
- `autobase.on('update')` reloads view on changes.
- Persists view via `open()` and handles operations in `apply()`.