Sets up **Hyperswarm** to connect peers and handle messages.

Create a new file named `network.js` in the project directory
```bash
mkdir src
touch network.js
```

Select and copy the full content below and paste it inside `network.js` file in your Pear app!
...
#### Key Functions:
- `initNetwork()` initializes a swarm.
- `joinTopic()` and `leaveTopic()` handle swarm topic.
- `broadcastToPeers()` sends messages to all peers.
- `handlePeerConnection()` replicates Corestore and listens to data.