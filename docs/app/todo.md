Manages the actual ToDo list data.

Create a new file named `todo-store.js` in the project directory
```bash
touch todo-store.js
```

Select and copy the full content below and paste it inside `todo-store.js` file in your Pear app!
...
#### Key Functions:
- `addTodo()` appends a new ToDo item to Autobase or UI (based on source).
- `deleteTodo()` removes a ToDo item by ID from Autobase or local UI.
- `toggleTodo()` flips the completed status of a ToDo via Autobase or UI.
- `loadTodosFromView()` rebuilds the current ToDo list by reading Autobase view.
- `renderTodos()` dynamically generates the ToDo list in the DOM.
- `updatePeerCount()` tracks and updates the number of connected peers.
- `getTodos()` returns a copy of the in-memory ToDo array.
