Handles peer invitation and pairing using **BlindPairing**.

**Pairing Flow**: A user creates an invite → another joins using `inviteHex` → pairing happens → joiner requests write access.

Create a new file named `pairing.js` in the project directory
```bash
touch pairing.js
```

Select and copy the full content below and paste it inside `pairing.js` file in your Pear app!
...
#### Key Functions:
- `createPairingInvite()` – Creates a new invite using the Autobase key.
- `joinWithInvite()` – Joins an invite and requests write access.
- `handleWriteAccessRequest(data)` – Adds writer when receiving a request.